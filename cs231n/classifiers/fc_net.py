...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        fwd, cache1 = affine_relu_forward(X, self.params['W1'], self.params['b1'])
        scores, cache2 = affine_forward(fwd, self.params['W2'], self.params['b2'])
        ...
        loss_reg = np.sum(np.square(self.params['W1']))+np.sum(np.square(self.params['W2']))
        loss = loss_data + 0.5 * self.reg * loss_reg

        # Compute grads
        dfwd, dW2, db2 = affine_backward(dscores, cache2)
        _, dW1, db1 = affine_relu_backward(dfwd, cache1)
